  ...
  int test_num = 50;
	...
	for (int i = 0; i < test_num; i++) {
		...
		if (action_channel->Receive(&msg) == kChannelStatusSuccess) {
			...
			void* src_addr = msg.allocate_memory.src_addr;
			void* data = malloc(data_size);	
			ibverbs_comm_net->DoRead(src_machine_id, src_addr, data, data_size, [&time_points, &bc, data, i, data_size](){
	...
	bc.WaitUntilCntEqualZero();
	double duration_sec = std::chrono::duration_cast<std::chrono::microseconds>(time_points[test_num] - time_points[0]).count();  
	std::cout << "the latency is : " << duration_sec/1000000.0 << " s, the throughput is : " 
		<< (1.0 * total_bytes) / duration_sec << "MB/s.\n" << "Test for correctness. Done.\n\n";
}
