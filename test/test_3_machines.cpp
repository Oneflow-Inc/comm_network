...
void TestThroughputWithBytes(uint64_t bytes, int64_t this_machine_id, Channel<Msg>* action_channel,
  ...
  BlockingCounter bc2(total_iteration);
  ...
  if (this_machine_id == 0) {
    ...
    for (int i = 0; i < total_iteration; i++) {
      ...
      msg.data_is_ready.data_size = bytes;
      msg.data_is_ready.src_machine_id = this_machine_id;
      msg.data_is_ready.dst_machine_id = 2;
      ibverbs_comm_net->SendMsg(2, msg);
      bc2.Decrease();
    }
    for (int i = 0; i < total_iteration; i++) {
      Msg msg;
  ...
  if (this_machine_id != 2) {
    bc.WaitUntilCntEqualZero();
  }
  if (this_machine_id != 0) {
    bc2.WaitUntilCntEqualZero();
  }
  if (this_machine_id != 0) {
...
int main(int argc, char* argv[]) {
  std::string ip_confs_raw_array[3] = {"192.168.1.12", "192.168.1.13", "192.168.1.15"};
  int32_t ctrl_port = 10534;
  std::vector<std::string> ip_confs(ip_confs_raw_array, ip_confs_raw_array + 3);
