...
void TestThroughputWithBytes(uint64_t bytes, int64_t this_machine_id, CommNet* comm_net) {
  ...
  std::vector<std::chrono::steady_clock::time_point> time_points(total_iteration + 10, std::chrono::steady_clock::now());
  int idx = 0;
...
void TestThroughput(int64_t this_machine_id, CommNet* comm_net) {
  ...
  uint64_t bytes = 2;
  for (int i = 0; i < 23; ++i) {
    TestThroughputWithBytes(bytes << i, this_machine_id, comm_net);
  }
  std::cout << "-------------------------------------------------------------------------------\n";
